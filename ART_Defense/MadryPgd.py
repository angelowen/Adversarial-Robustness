...
print("AdversarialTraining~")


# Adversarial Training Fast is Better than Free
Madry = AdversarialTrainerMadryPGD(classifier,nb_epochs =200,eps = 25, batch_size =512)
Madry.fit(x_train, y_train)
mymodel = Madry.get_classifier()
predictions = mymodel.predict(x_test)
accuracy = np.sum(np.argmax(predictions, axis=1) == np.argmax(y_test, axis=1)) / len(y_test)
print("Accuracy on benign test examples: {}%".format(accuracy * 100))

attack = FastGradientMethod(estimator=classifier, eps=16/255)
x_test_adv = attack.generate(x=x_test)

predictions = mymodel.predict(x_test_adv)
accuracy = np.sum(np.argmax(predictions, axis=1) == np.argmax(y_test, axis=1)) / len(y_test)
print("Accuracy on adversarial test examples: {}%".format(accuracy * 100))
