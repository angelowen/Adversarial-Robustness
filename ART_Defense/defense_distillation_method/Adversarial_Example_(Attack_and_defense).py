...
def defense(device,train_loader,val_loader,test_loader,epochs,Temp,epsilons):
  ...
  plt.legend()
  plt.savefig(f"images/Network_F")
  # plt.show()
  ...
  plt.xlabel("Num of epochs")
  plt.legend()
  plt.savefig(f"images/Network_F1")
  # plt.show()

  model2 = NetF().to(device)
  model2.load_state_dict(modelF.state_dict())
  model = NetF1().to(device)
  model.load_state_dict(modelF1.state_dict())
  torch.save(model, 'defense.pth')
  for attack in ("fgsm","ifgsm","mifgsm"):
    ...
    for eps in epsilons:
        print("No Defense:")
        acc_1, ex = test(model2,device,test_loader,eps,1,attack)
        accuracies_attack.append(acc_1)  
        print("Defense:")
    ...
    plt.plot(epsilons, accuracies_attack, "*-")
    plt.title(attack)
    plt.xlabel("Epsilon")
    plt.ylabel("Accuracy")
    plt.savefig(f"images/Attack_{attack}")
    ...
    plt.xlabel("Epsilon")
    plt.ylabel("Accuracy")
    plt.savefig(f"images/Defense_{attack}")
    # plt.show()
    ...
    plt.tight_layout()
    plt.savefig(f"images/Image_{attack}")
    # plt.show()
